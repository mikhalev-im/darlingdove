...
export function* addToCart({ productId, qty = 1 }: AddToCartAction) {
  try {
...
export function* addPromocode({ code }: AddPromocodeAction) {
  // get cartId from store
  const cartId = yield select(getCartId);
  try {
    // make api request
    ...
    if (PROMO_ERROR_CODE_MESSAGES[err.code]) {
      message = PROMO_ERROR_CODE_MESSAGES[err.code] as string;
    } else {
...
export function* removeFromCart({ productId }: RemoveFromCartAction) {
  try {
    // get cartId from store
    const cartId = yield select(getCartId);
    // make api request
